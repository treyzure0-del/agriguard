...
function renderForecast(data) {
  ...
  data.daily.time.forEach((dateStr, i) => {
    ...
    card.innerHTML = `
      ...
      <span class="forecast-icon-text">${icon}</span>
      <div class="forecast-high">${high}°</div>
      <div class="forecast-low">${low}°</div>
      ${rainProb > 20 ? `<div class="forecast-rain">OK️ ${rainProb}%</div>` : '<div class="forecast-rain" style="opacity:0">-</div>'}
    `;
...
function renderHourly(data) {
  ...
  for (let i = startIdx; i < endIdx; i++) {
    ...
    card.innerHTML = `
      ...
      <span class="hourly-icon-text">${icon}</span>
...
function set(id, value) {
  ...
  if (!el) return;
  // Weather icon IDs get special badge treatment
  const iconIds = ['weatherBigIcon', 'homeWeatherIcon', 'wAdviceIcon'];
  if (iconIds.includes(id)) {
    el.innerHTML = '<span class="weather-condition-badge">' + value + '</span>';
  } else {
